...
const NAV = [
	{ key: "accueil", label: "Accueil", icon: (
		<svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" className="text-slate-500"><path d="M3 12l9-9 9 9" /><path d="M9 21V9h6v12" /></svg>
	) },
	{ key: "fiche", label: "Fiche perception", icon: (
		<svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" className="text-slate-500"><rect x="4" y="4" width="16" height="16" rx="2" /><path d="M8 2v4M16 2v4M4 10h16" /></svg>
	) },
	{ key: "logs", label: "Logs", icon: (
		<svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" className="text-slate-500"><rect x="4" y="4" width="16" height="16" rx="2" /><path d="M8 2v4M16 2v4M4 10h16" /></svg>
	) },
	{ key: "admin", label: "Admin", icon: (
		<svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" className="text-slate-500"><circle cx="12" cy="12" r="3" /><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 1 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 1 1-4 0v-.09a1.65 1.65 0 0 0-1-1.51 1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 1 1-2.83-2.83l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 1 1 0-4h.09a1.65 1.65 0 0 0 1.51-1 1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 1 1 2.83-2.83l.06.06a1.65 1.65 0 0 0 1.82.33h.09a1.65 1.65 0 0 0 1-1.51V3a2 2 0 1 1 4 0v.09a1.65 1.65 0 0 0 1 1.51h.09a1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 1 1 2.83 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82v.09a1.65 1.65 0 0 0 1.51 1H21a2 2 0 1 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z" /></svg>
	) }
];

export default function MenuArmurerie({ page, setPage, onLoginClick }) {
	const { session, logout } = useAdminData();
	const isLoggedIn = session && session.role && session.role !== "invite";

	// Register global fallback for login modal
	useEffect(() => {
		if (typeof window !== 'undefined' && typeof onLoginClick === 'function') {
			window.setArmurerieShowLogin = onLoginClick;
		}
		return () => {
	...
	// Filtrage des onglets selon le rôle (mêmes règles que MenuComptoire)
	let navFiltered = NAV;
	if (session) {
		if (session.role === "invite") {
			navFiltered = NAV.filter((item) => ["accueil", "fiche", "logs"].includes(item.key));
		} else if (session.role === "gestionnaire") {
			navFiltered = NAV.filter((item) => ["accueil", "fiche", "logs"].includes(item.key));
		} else if (session.role === "admin") {
			navFiltered = NAV.filter((item) => ["accueil", "fiche", "logs", "admin"].includes(item.key));
	...
	return (
		<div className="w-full">
			{/* Header identique MenuComptoire */}
			<div className="bg-[#14213d] text-white flex items-center justify-center py-3 px-4 w-full">
				<div className="max-w-4xl w-full mx-auto flex items-center justify-center relative group">
					<span className="flex items-center gap-2 text-xl font-bold mx-auto">
						...
						<span>Armurerie PSIG - Gestion interne</span>
						{/* Gendarme SVG à droite */}
					...
					<div className="flex items-center gap-2">
						{session && session.role === "invite" && (
							<>
								...
								<button
									onClick={onLoginClick || (() => { if (typeof window !== 'undefined' && typeof window.setArmurerieShowLogin === 'function') window.setArmurerieShowLogin(true); })}
