...
const NAV = [
	{
		...
		label: "Véhicules",
		icon: (
			<svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" className="text-blue-500"><rect x="3" y="7" width="18" height="10" rx="2"/><circle cx="7" cy="19" r="2"/><circle cx="17" cy="19" r="2"/></svg>
		),
	},
	{
		key: "logs",
		label: "Logs",
		icon: (
			<svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" className="text-blue-500"><rect x="4" y="4" width="16" height="16" rx="2"/><path d="M8 2v4M16 2v4M4 10h16"/></svg>
		),
	},
	{
		key: "admin",
		label: "Admin",
		icon: (
			<svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24" className="text-blue-500"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 1 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 1 1-4 0v-.09a1.65 1.65 0 0 0-1-1.51 1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 1 1-2.83-2.83l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 1 1 0-4h.09a1.65 1.65 0 0 0 1.51-1 1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 1 1 2.83-2.83l.06.06a1.65 1.65 0 0 0 1.82.33h.09a1.65 1.65 0 0 0 1-1.51V3a2 2 0 1 1 4 0v.09a1.65 1.65 0 0 0 1 1.51h.09a1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 1 1 2.83 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82v.09a1.65 1.65 0 0 0 1.51 1H21a2 2 0 1 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z"/></svg>
		),
	},
];

export default function MenuVehicule({ ...props }) {
	...
	if (session) {
		if (session.role === "invite") {
			navFiltered = NAV.filter((item) => ["vehicule", "logs"].includes(item.key));
		} else if (session.role === "gestionnaire") {
			navFiltered = NAV.filter((item) => ["vehicule", "logs", "admin"].includes(item.key));
	...
	return (
		<div className="w-full">
			...
			<div className="bg-[#14213d] text-white flex items-center justify-center py-3 px-4 w-full">
				<div className="max-w-4xl w-full mx-auto flex items-center justify-center relative group">
      ...
      <path d="M16 5V27" stroke="#fff" strokeWidth="1.2" strokeDasharray="2 2"/>
					  ...
					  <span>Vehicule PSIG - Gestion interne</span>
      ...
      <ellipse cx="16" cy="13" rx="2.2" ry="2.5" fill="#fff" opacity=".15"/>
					...
					{/* Flèche retour accueil PSIG, n'apparaît qu'au survol du header, et seulement si window.setAccueilPSIG est une fonction */}
					{typeof window !== 'undefined' && typeof window.setAccueilPSIG === 'function' && (
					  <button
					    className="absolute left-0 top-1/2 -translate-y-1/2 bg-transparent rounded-full p-1 transition-all border-none shadow-none opacity-0 group-hover:opacity-100 focus:opacity-100 hover:bg-blue-100/60 outline-none"
					    onClick={() => window.setAccueilPSIG()}
					    style={{zIndex: 10, outline: 'none', border: 'none'}}
					    aria-label="Retour à l'accueil PSIG"
					    tabIndex={0}
					  >
					    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#1e293b" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" className="transition"><path d="M15 19l-7-7 7-7"/></svg>
					  </button>
					)}
				</div>
			</div>
				...
				<div className="max-w-4xl w-full mx-auto flex items-center justify-between">
					<div className="flex flex-wrap gap-1 sm:gap-2">
						{navFiltered.map((item) => (
							<button
								key={item.key}
								className={`px-3 py-1.5 rounded-md font-medium transition-all duration-150 flex items-center gap-1 focus:outline-none focus:ring-1 focus:ring-blue-400 text-blue-800/90 ${
									props.page === item.key
